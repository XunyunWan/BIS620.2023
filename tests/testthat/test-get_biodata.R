test_that("get_biodata returns expected columns", {
  ...
  result <- get_biodata(dl_mock)
  expect_true(all(c("SUBJID", "ATRT", "DTHDY", "DTH", "AGE", "SEX", "B_WEIGHT", "B_HEIGHT", "RACE", "BMI", "KRAS") %in% names(result)))
})
